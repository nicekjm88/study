...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const initScrollTrigger = (element) => {
    ScrollTrigger.create({
      trigger: element, // 이벤트가 시작되는 요소
      start: "top center", // 트리거가 뷰포트 상단 중앙에 도달했을 때 시작
      end: "+=900",
      onEnter: () => element.classList.add('inactive'), // 지정된 지점에 도달하면 inactive 클래스 추가
      onEnterBack: () => element.classList.add('inactive'), // 지정된 지점에 도달하면 inactive 클래스 추가
      onLeave: () => element.classList.remove('inactive'), // 지정된 지점을 벗어나면 inactive 클래스 제거
      onLeaveBack: () => element.classList.remove('inactive'), // 지정된 지점을 벗어나면 inactive 클래스 제거
      // markers: true // 개발 중 마커를 통해 시작과 끝 지점 시각화 (개발 완료 후 제거)
    });
  };
